    ...
    <form class="form-inline float-right" method="GET" action="{{ route('mantenedor_carga_parentesco') }}">
        <div class="input-group mb-2 mr-sm-2">
        ...
        <div class="input-group mb-2 mr-sm-2">
            <select name="columna" id="columna" class="form-control form-control-sm">
                ...
                <option value="nombre" @if(request('columna') === 'nombre') {{ 'selected' }} @endif>Parentesco</option>                           
            </select>
        </div>
        ...
        <input type="hidden" name="buscar_parentesco" value="{{ request('buscar_parentesco') }}">
