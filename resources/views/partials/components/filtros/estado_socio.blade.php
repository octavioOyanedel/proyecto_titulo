    ...
    <form class="form-inline float-right" method="GET" action="{{ route('mantenedor_socio_estado') }}">
        <div class="input-group mb-2 mr-sm-2">
        ...
        <div class="input-group mb-2 mr-sm-2">
            <select name="columna" id="columna" class="form-control form-control-sm">
                ...
                <option value="nombre" @if(request('columna') === 'nombre') {{ 'selected' }} @endif>Estado socio</option>                           
            </select>
        </div>
        ...
        <input type="hidden" name="buscar_estado_socio" value="{{ request('buscar_estado_socio') }}">
