    ...
    <form class="form-inline float-right" method="GET" action="{{ route('mantenedor_contable_tipo_cuenta') }}">
        <div class="input-group mb-2 mr-sm-2">
        ...
        <div class="input-group mb-2 mr-sm-2">
            <select name="columna" id="columna" class="form-control form-control-sm">
                ...
                <option value="nombre" @if(request('columna') === 'nombre') {{ 'selected' }} @endif>Tipo de cuenta</option>                           
            </select>
        </div>
        ...
        <input type="hidden" name="buscar_tipo_cuenta" value="{{ request('buscar_tipo_cuenta') }}">
