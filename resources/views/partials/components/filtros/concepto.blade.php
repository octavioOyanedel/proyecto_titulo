    ...
    <form class="form-inline float-right" method="GET" action="{{ route('mantenedor_contable_concepto') }}">
        <div class="input-group mb-2 mr-sm-2">
        ...
        <div class="input-group mb-2 mr-sm-2">
            <select name="columna" id="columna" class="form-control form-control-sm">
                ...
                <option value="nombre" @if(request('columna') === 'nombre') {{ 'selected' }} @endif>Nombre</option>  
                <option value="tipo_registro_contable_id" @if(request('columna') === 'tipo_registro_contable_id') {{ 'selected' }} @endif>Tipo Registro</option>                         
            </select>
        </div>
        ...
        <input type="hidden" name="buscar_concepto" value="{{ request('buscar_concepto') }}">
