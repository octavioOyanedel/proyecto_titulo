...
<div class="form-group row">
    ...
    <div class="col-md-6">
        <select id="tipo_registro_contable_id" class="default-selects form-control @error('tipo_registro_contable_id') is-invalid @enderror" name="tipo_registro_contable_id" autocomplete="tipo_registro_contable_id" autofocus required>
            ...
            @if(old('tipo_registro_contable_id') === null)
                ...
                @foreach($tipos_registro as $t)
                    @if(isset($concepto))
                        ...
                        <option value="{{ $t->id }}" {{ $concepto->getOriginal('tipo_registro_contable_id') == $t->id ? 'selected' : ''}}>{{ $t->nombre }}</option>
                    @else
                        <option value="{{ $t->id }}" {{ $tipo_registro_contable_id == $t->id ? 'selected' : ''}}>{{ $t->nombre }}</option>
                    @endif
                @endforeach
            @else
                {{-- loop con old --}}
